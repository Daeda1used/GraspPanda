...
def slots(method):
    ...
    if method in ('hggd','region_normalized_grasp'):
        return (ComponentSlot('backbone','backbone',('upstream','native_resnet','convnextv2','repvit','mobilenetv4','dinov2','dinov3','vmamba','rala','mambavision','efficientvit','fastvit'),
...
def configure_model(model,method,selection,voxel_size=.005):
    ...
    for slot in slots(method):
        ...
        elif method in ('hggd','region_normalized_grasp') and choice == 'fastvit':
            from .modules.fastvit import FastViTPyramid
            replacement = FastViTPyramid(**options)
...
def load_checkpoint(model,state,changed_prefixes=(),policy='strict'):
    ...
    for prefix in changed_prefixes:
        ...
        if isinstance(module, (DinoPyramid, MambaVisionPyramid, EfficientViTPyramid, FastViTPyramid, FoundationBackbone, SparseFoundationBackbone)):
